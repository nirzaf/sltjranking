...
else 
{
    if (isset($_POST['add'])) { 
        ...
        $DoneBy = $_POST['doneby'];
        $Crowd = $_POST['crowd'];
        $sql = "INSERT INTO tblstudents(StudentId,EmailId,MobileNumber,Done_By,Status,Coun,EventDate,Points,Crowd) VALUES(:EventID,:EventName,:BranchName,:Done_By,:Status,:Count,:EvenDate,:Points,:Crowd)";
        $query = $dbh->prepare($sql);
        $query->bindParam(':EventID', $eventID, PDO::PARAM_STR);
        $query->bindParam(':EventName', $eventName, PDO::PARAM_STR);
        $query->bindParam(':BranchName', $author, PDO::PARAM_STR);
        $query->bindParam(':Done_By', $DoneBy, PDO::PARAM_STR);
        $query->bindParam(':Status', 0 , PDO::PARAM_INT);
        $query->bindParam(':Count', $Count, PDO::PARAM_STR);
        $query->bindParam(':EventDate', $Date, PDO::PARAM_STR);
        $query->bindParam(':Points', $Points, PDO::PARAM_STR);
        $query->bindParam(':Crowd', $Crowd, PDO::PARAM_STR);
        $query->execute();
